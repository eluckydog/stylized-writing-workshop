...
| 库 | 大小 | 内容 |
|----|------|------|
| `vector_db/maqianzu/` | **177MB** | **睡前消息832期 + 高见40篇 + 参考信息446篇 + 黑话60篇 = 36,303条语义切片（细粒度 ~239字/块）** |
| `vector_db/lukewen/` | **30MB** | **卢克文444篇文章 = 12,164条语义切片** |
| `vector_db/jiubian/` | **73MB** | **九边公众号562篇文章 = 12,389条语义切片** |
...
| 写手 | 论证结构 | 向量数据 | 说明 |
|------|---------|---------|------|
| **马前卒** | 问题→数据→分析→方案 | ✅ **36,303条** | 工程师视角，数据驱动 |
| **卢克文** | 地缘→冲突→推演→前景 | ✅ **12,164条** | 宏大叙事，地缘政治 |
| **九边** | 背景→现象→深度→开放 | ✅ **12,389条** | 长线社会观察，认知升级视角 |
| **国家地理** | 发现→原理→数据→意义 | ✅ 成语+诗词 | 科学叙事 |

向量库定位说明：三巨头语料（马前卒/卢克文/九边）是**方法论标本库**——既可在写作时检索「已覆盖话题」的真实原文作为证据补充，也可离线蒸馏出各自的「招牌论证动作 + 思维链 + 禁忌」，用于分析尚未覆盖的新话题（详见 `methodology/`）。
...
├── vector_db/                      向量库 (LFS)
├── methodology/                    方法论蒸馏层
│   ├── distill.py                  离线蒸馏三巨头方法论画像
│   ├── methodology_engine.py       方法论迁移：套到任意新题生成骨架
│   └── profiles/                   <writer>_profile.json（可检视）
└── skills/styles/                  风格参考
```
...
本项目的目标不是"检索旧文章"，而是**从语料里提炼"怎么分析一个问题"的方法论**，再把它迁移到**尚未出现的新话题**上。

- **RAG 库定位为配角**：对已覆盖话题，检索真实原文作证据补充；对未覆盖话题，检索必然空手（逻辑必然），但写作照常进行。
- **方法论画像为主角**：`methodology/distill.py` 从三库**数据驱动**抽取每位写手的招牌标记词频率、高频开场句式、真实论点句样本，合并手写论证骨架，落盘为 `methodology/profiles/<writer>_profile.json`（完全可检视、非黑盒）。
- **迁移用法**：

```python
from methodology.methodology_engine import instantiate, format_scaffold

# 即使"量子计算"在九边语料中从未出现，也能生成方法论骨架
scaffold = instantiate("jiubian", "2026 年的量子计算对普通人意味着什么")
print(format_scaffold(scaffold))
# → 四段骨架（背景→现象→深度→开放）+ 角度风格样本
# → rag_evidence 为空（未覆盖），凭方法论骨架写作
```

`research_orchestrator.write()` 已内置注入方法论骨架，LLM 写作时直接拿到"该风格怎么想"的论证指引。
